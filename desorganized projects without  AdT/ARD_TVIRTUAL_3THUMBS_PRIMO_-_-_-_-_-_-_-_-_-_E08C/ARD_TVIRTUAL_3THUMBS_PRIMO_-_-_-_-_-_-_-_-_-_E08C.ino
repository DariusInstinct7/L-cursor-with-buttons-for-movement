...
void loop(){
   ...
   if(num!=anterior){ //si se cambio el numero en THUMBSWITCH ENTONCES RECIEN RECALCULO TODO :
      ...
      Serial.print(num);
      if(num==1 || num==0){ //NUEVO NUMERO , NO ES PRIMO
         Serial.println(" no es primo.");
      }
      else{//casos>1
         for(i=2;i<num;i++){ //divididos por todos los numeros
            if(num%i==0){algunavez++;} //debe haber CERO divisores para ser primo
         }
         if(algunavez==0){Serial.println(" es primo.");}
         else{Serial.println(" no es primo.");}
      }
      anterior=num;
      algunavez=0;
